    ...
    mSpeakerDrcEnabled(false)
{
    ...
    // author: zxchen, purpose: audio output priority is configurable.
    property_get("ro.audio.output.priority", value, "hdmi");
	...
	// author: yuchen, purpose: audio remote output(WFD) priority is configurable.
    property_get("ro.audio.output.priority2", value, "remote");
    if(strncmp(value, "earphone", 8) == 0) {
		// 1: earphone > hdmi
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
	    // AcitonsCode(author: zxchen, type:change_method)
        uint32_t device2 = AUDIO_DEVICE_NONE;
        if (strategy != STRATEGY_SONIFICATION) {
            // no sonification on remote submix (e.g. WFD)
            // edited: yuchen, purpose: with remote output(WFD) priority, priority bigger than 2 means earphone over remote.
        ...
        // edited: yuchen, purpose: with remote output(WFD) priority, odd number means earphone over hdmi.
        ...
        // edited: yuchen, purpose: with remote output(WFD) priority, even number means hdmi over earphone.
                                             ...
                                             int delayMs)
{
    ...
    /*
     * FIXME
    if ((device == AUDIO_DEVICE_OUT_AUX_DIGITAL) && (prevDevice == AUDIO_DEVICE_OUT_AUX_DIGITAL)) {
        // when in hdmi mode, should reconfigure hdmi audio, in case audio packeted wrongly.
        // plug HDMI or change HDMI conf, without stoping music(4.1), this check is needed.
        // otherwise(4.2), doing this check is needless.
	} else */
                                            ...
                                            audio_devices_t device)
{
    ...
    // author:zxchen, purpose: media volume is not fixed to max in hdmi mode.
    // update its counterpart in \frameworks\base\media\java\android\media\AudioService.java
